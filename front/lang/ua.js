export default {
  lang: {
    ru: "Російська",
    ua: "Українська",
    en: "Англійська",
  },
  copyUrl: "Копіювати",

  title: {
    first: "Залишайтеся на зв’язку з родиною та друзями ",
    second: "з Інтернетом або без нього",
  },
  connected: "Ваш нинішній сервер <b>{name}</b>",
  subtitle: "Децентралізовані інструменти комунікації, які ви зможете використовувати, навіть якщо доступ до Telegram, Whatsapp, Facebook та інших мереж буде недоступним.",
  server: "ІНШІ СЕРВЕРИ:",
  alert: {
    title: "Не зволікай! Використовуй вже!",
    descr: "Створюйте власні мережі за допомогою коду. Приєднуйтесь до технічної групи для обговорення співпраці, нових послуг та волонтерства",
    buttons: {
      repo: "Репозиторій",
      servers: "Matrix cервер",
    },
  },
  cities: {
    kyiv: "Київ",
    odessa: "Одеса",
    kharkiv: "Харків",
    lviv: "Львів",
    lviv2: "Львів 2",
    rivne: "Рівне",
    kherson: "Херсон",
    mykolayiv: "Миколаїв",
    poltava: "Полтава",
  },

  footer: {
    initiative: `Ініціатива <a target="_blank" href="https://equalit.ie/">eQualit.ie</a>`,
    license: `За ліцензією <a  href="https://github.com/censorship-no/dcomms"> GNU General Public License v3.0</a>`,
  },
  rubric: {
    messaging: "Переписка",
    browsing: "Перегляд сайтів",
    social: "Соцмережі",
    //    filesharing: "Файлообмінник",
  },
  adressCopy: "Адрес копірований",
  items: {
    desktopApp: "Як отримати доступ з комп’ютера (десктоп):",
    localDownload: "локальне скачування",
    instruction: "інструкція:",
    downloads: "Завантажити:",
    mobileApp: "Додаток для смартфона:",
    notAvail: "Недоступний",
  },
};
